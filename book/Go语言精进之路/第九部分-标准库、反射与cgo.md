...
## 59 谨慎使用 reflect 包提供的反射能力

> “反射并不是 Go 推荐的惯用法，建议大家谨慎使用”——Rob Pike

**反射**是程序在运行时访问、检测和修改它本身状态或行为的一种能力。

Go 的反射本质上就是利用 interface{} 在运行时对任意变量的**类型和值信息进行检视甚至是对值进行修改**的机制。
...
- 修改反射对象的前提：反射对象对应的 reflect.Value 必须是可设置的（Settable），当被反射对象以值类型（T）传递给 reflect.ValueOf 时，在反射世界中对反射对象值信息的修改不会对被反射对象产生影响。Go 中一旦发生这种行为，将会导致运行时 panic。

reflect.Value 提供了 CanSet、CanAddr、CanInterface 等方法来帮助判断反射对象是否可设置、可寻址、可恢复。

## 60 了解 cgo 的原理和使用开销

Go 源码中的 C 代码需要用注释包裹，紧接着（不能有空行）必须有 `import "C"` 语句。C 可以理解为伪包名，C 语言所有语法元素均在该伪包下面。

实际编译过程中，go build 调用了名为 cgo 的工具，cgo 会识别和读取 Go 源文件中的 C 代码，并将其提取后交给外部的 C 编译器（clang 或 gcc）编译，最后与 Go 源码编译后的目标文件链接成一个可执行程序。

**对于在 C 内部分配的内存，Go 中的 GC 是无法感知到的，因此要记着在使用后手动释放**。

使用 cgo 的开销：调用开销（将代码尽量下推到 C 中以减少语言间交互调用的次数，从而降低平均调用开销）；增加线程数量暴涨的可能性（Go 优化了一些原本会导致线程阻塞的系统调用，但是 Go 调度器无法掌控 C 世界）；失去跨平台交叉构建能力；Go 垃圾回收，C 手工内存管理等。

**静态构建**就是指构建后的应用运行所需的所有符号、指令和数据都包含在自身的二进制文件当中，没有任何对外部动态共享库的依赖。静态构建出的二进制文件由于包含所有符号、指令和数据，因而通畅要比非静态构建的应用大许多。默认情况下，Go 没有采用静态构建。

Go 链接器支持两种工作模式：内部链接和外部链接。

- 如果你的程序用了 net、os/user 等几个依赖 cgo 实现的包之外的 Go 包，那么你的程序默认将是纯静态的，不依赖任何 C 运行时库等外部动态链接库；
- 如果使用了 net 这样的包含 cgo 实现版本的标准库包，那么 CGO_ENABLED 的值将影响你的程序编译后的属性（静态/动态链接）；
- 如果 CGO_ENABLED=1 且进使用了 net、os/user 等依赖实现的包，那么内部依赖机制将被默认采用，编译过程不会采用静态链接；但如若依然要强制静态编译，需向 `-ldflags '-linkmode "external" -extldflags "-static"'` 传递 go build 命令。
